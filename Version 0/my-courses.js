...
document.addEventListener("DOMContentLoaded", () => {
  ...
  payCertificationFeeButton.addEventListener("click", () => {
    ...
    const redirect_url = `https://lms-software-two.vercel.app/my-courses.html?courseid=${courseId}&teachableid=${teachable}&amount=${amount}`;
...
//remove pay certificcation certificate
document.addEventListener("DOMContentLoaded", () => {
  // Get the "Pay Certification Fee" and "Begin Capstone Project" buttons
  const payCertificationFeeButton = document.querySelector(
    ".course_page_search button:nth-child(1)"
  );
  const beginCapstoneProjectButton = document.querySelector(
    ".course_page_search button:nth-child(2)"
  );
  ...
  if (status && tx_ref && transaction_id) {
    // Hide the "Pay Certification Fee" button
    payCertificationFeeButton.style.display = "none";

    // Enable the "Begin Capstone Project" button
